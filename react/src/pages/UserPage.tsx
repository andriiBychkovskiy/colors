...
const CursorSizeButton = styled(Button)<{ brushSize: number }>(
  ({ brushSize }) => ({
...
const UserPage: React.FC = () => {
  ...
  const fetchProgress = async (userId: string, svgId: string) => {
    try {
      const response = await axios.get(
        `${API_URL}/progress?userId=${userId}&svgId=${svgId}`
      );
      if (response.data.length > 0) {
        setProgress(response.data[0]); // Устанавливаем прогресс, если он найден
      } else {
        setProgress({
          userId,
          svgId,
  ...
  useEffect(() => {
    const saveProgress = async () => {
      if (progress.userId && progress.svgId) {
        ...
        try {
          if (progress.id) {
            await axios.put(`${API_URL}/progress/${progress.id}`, progress);
          } else {
  ...
  const fetchAllUserProgress = async (userId: string) => {
    try {
      const response = await axios.get(`${API_URL}/progress?userId=${userId}`);
  ...
  const handleClearAll = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `${API_URL}/progress?userId=${user.id}&svgId=${selectedImage.id}`
      );
      const progressEntries = response.data;

      // Delete each progress entry
      for (const entry of progressEntries) {
        await axios.delete(`${API_URL}/progress/${entry.id}`);
      }
